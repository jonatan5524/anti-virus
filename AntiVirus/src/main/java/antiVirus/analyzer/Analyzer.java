...
//	private String initScanningDirectory;
	...
	private void runOnDbRepositoryFileAndAnalyze(FileDB tempFileDB) throws AntiVirusAnalyzeException {
		logger.info("analyzing file: " + tempFileDB.getPath());
		Optional<resultScanStatus> resultScanStatusOptinal = Optional
				.ofNullable(tempFileDB.getResultScan().getResult());
	...
	private void analyzeFileUpdateResultScan(FileDB tempFileDB) throws AntiVirusAnalyzeException {
		...
		try {
			...
			throw new AntiVirusAnalyzeException(
					"exception at deserializing or serializing file " + tempFileDB.getPath(), e);
		}
	}

	private int analyzeSingleFile(FileDB tempFileDB) {
		int analyzeCounter = 0;
		boolean result;
	...
	private List<FileDB> waitForDbRepo(List<FileDB> DBlist, int index) throws AntiVirusAnalyzeException {
		...
		while (needToWaitForList(index)) {
			try {
				...
				throw new AntiVirusAnalyzeException("interrupted while waiting for file repository", e);
			}
		}
	...
	private boolean needToWaitForList(int index) {
		...
		long count;
		if (fileFolderScanner.isScanningAllDrives()) {
			count = fileRepoTemp.count();
		} else {
			count = fileRepoTemp.countByPathStartingWith(
					Utils.convertPathToValidMySQLSearch(fileFolderScanner.getInitScanningDirectory()));
		}
	...
	private List<FileDB> getDBListFromRepo() {
		...
		if (fileFolderScanner.isScanningAllDrives()) {
			...
			return fileFolderScanner.getFileRepo().findByPathStartingWith(
					Utils.convertPathToValidMySQLSearch(fileFolderScanner.getInitScanningDirectory()));
		}
